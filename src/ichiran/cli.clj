(ns ichiran.cli
  "Minimal JVM entrypoint for `ichiran.query` against an EDN-seeded MemStore —
  ...
  Usage: `clojure -M -m ichiran.cli <ledger.edn> <artifact-id>` — prints the
  draft status (\"proposed\"/\"published\"/\"none\") and exits 0 on
  \"published\", 1 otherwise (so callers can also just check the exit code).

  <ledger.edn> holds the same shape as `ichiran.store/demo-data`'s :artifacts
