(ns ichiran.kotoba
  ...
  itself is already proven backend-agnostic (test/ichiran/store_contract_test)."
  (:require [clojure.string :as str]
            [langchain.kotoba-db :as kdb]
            [ichiran.cacao :as cacao]
...
(defn kotoba-store
  "A `store/DatomicStore` backed by a kotoba-server pod (e.g. kotobase.net).
   Auth options (pick one):
     ...
     :identity         an ichiran.cacao identity ({:private-key :public-key :did})
