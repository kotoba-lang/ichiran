(ns ichiran.cacao-test
  ...
  per-actor key model is the ichiran/teian/kekkai/tayori/koyomi analog — the
  key IS the identity."
...
(deftest minted-cacao-signature-verifies
  (testing "the Ed25519 signature is over the exact SIWE message"
    (let [id    (c/generate-identity)
          grant {:cap :cap/read :scope "ichiran"}
...
(deftest minted-cacao-is-wellformed-cbor
  (let [id    (c/generate-identity)
        cacao (c/mint id {:cap :cap/transact :scope "ichiran"}
...
(deftest per-actor-key-persists
  (testing "load-or-create! generates once, then reloads the same identity"
    (let [p  (str (System/getProperty "java.io.tmpdir") "/ichiran-id-test-" (System/nanoTime) ".edn")
...
(deftest resources-encode-capability-and-graph
  (is (= ["kotoba://op/datom:read" "kotoba://graph/ichiran"]
         (c/grant->resources {:cap :cap/read :scope "ichiran"})))
